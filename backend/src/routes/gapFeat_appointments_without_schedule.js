...
// Gap Feature: Appointments without \'/schedule
// Appointments without /schedule-optimization (balance clinic capacity, no-show prediction)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIOptometryAssistant / gap-appointments-without-schedule',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Appointments without \'/schedule'.\nProject: AIOptometryAssistant.\nContext: Appointments without /schedule-optimization (balance clinic capacity, no-show prediction)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['appointments-without-schedule', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'appointments-without-schedule', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
