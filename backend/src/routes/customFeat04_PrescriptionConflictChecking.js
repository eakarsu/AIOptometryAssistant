...
// Cross-reference patient Rx against medications in medical history; flag interactions (dilated drops + driving, etc.)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIOptometryAssistant / cf-prescription-conflict-checking',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_prescription_conflict_checking`;
try {
  await pool.query(`
...
router.post('/run', authMiddleware, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Prescription conflict checking'.
Context: Cross-reference patient Rx against medications in medical history; flag interactions (dilated drops + driving, etc.)
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-prescription-conflict-checking/run error:', err);
