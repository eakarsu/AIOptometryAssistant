...
// Gap Feature: Patients without \'/patient
// Patients without /patient-risk-score (predict disease progression, glaucoma risk)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIOptometryAssistant / gap-patients-without-patient',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Patients without \'/patient'.\nProject: AIOptometryAssistant.\nContext: Patients without /patient-risk-score (predict disease progression, glaucoma risk)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['patients-without-patient', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'patients-without-patient', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
