...
// Background process monitors patients due for exams; auto-sends SMS/email reminders; predicts best contact method and optimal timing
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIOptometryAssistant / cf-agentic-patient-follow-up',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_agentic_patient_follow_up`;
try {
  await pool.query(`
...
router.post('/run', authMiddleware, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Agentic patient follow-up'.
Context: Background process monitors patients due for exams; auto-sends SMS/email reminders; predicts best contact method and optimal timing
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-agentic-patient-follow-up/run error:', err);
