...
// Patient uploads eye selfie; AI pre-screens for obvious concerns (cataracts, pupil asymmetry); triage to doctor
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIOptometryAssistant / cf-computer-vision-screening-automation',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_computer_vision_screening_automation`;
try {
  await pool.query(`
...
router.post('/run', authMiddleware, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Computer vision screening automation'.
Context: Patient uploads eye selfie; AI pre-screens for obvious concerns (cataracts, pupil asymmetry); triage to doctor
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-computer-vision-screening-automation/run error:', err);
