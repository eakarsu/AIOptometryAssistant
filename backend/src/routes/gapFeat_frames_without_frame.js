...
// Gap Feature: Frames without \'/frame
// Frames without /frame-style-suggest (personalized recommendations based on demographics)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIOptometryAssistant / gap-frames-without-frame',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Frames without \'/frame'.\nProject: AIOptometryAssistant.\nContext: Frames without /frame-style-suggest (personalized recommendations based on demographics)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['frames-without-frame', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'frames-without-frame', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
