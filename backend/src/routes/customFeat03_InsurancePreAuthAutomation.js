...
// Detect insurance-dependent treatments; auto-generate and submit pre-auth requests; track approval status
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIOptometryAssistant / cf-insurance-pre-auth-automation',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_insurance_pre_auth_automation`;
try {
  await pool.query(`
...
router.post('/run', authMiddleware, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Insurance pre-auth automation'.
Context: Detect insurance-dependent treatments; auto-generate and submit pre-auth requests; track approval status
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-insurance-pre-auth-automation/run error:', err);
