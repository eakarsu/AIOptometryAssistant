...
// Gap Feature: Limited EHR integration (some integration stubs but no Epic/Allscripts adapter)
// Limited EHR integration (some integration stubs but no Epic/Allscripts adapter)
// Project: AIOptometryAssistant
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIOptometryAssistant / gap-limited-ehr-integration-some-integration-stubs-but',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Limited EHR integration (some integration stubs but no Epic/Allscripts adapter)'.\nProject: AIOptometryAssistant.\nContext: Limited EHR integration (some integration stubs but no Epic/Allscripts adapter)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['limited-ehr-integration-some-integration-stubs-but', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'limited-ehr-integration-some-integration-stubs-but', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['limited-ehr-integration-some-integration-stubs-but']
    );
