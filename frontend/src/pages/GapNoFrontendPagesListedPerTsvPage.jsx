...
// Gap (Non-AI): No frontend pages listed (per TSV)
// No frontend pages listed (per TSV) — UI coverage unclear
import React, { useState } from 'react';

export default function GapNoFrontendPagesListedPerTsvPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-frontend-pages-listed-per-tsv/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No frontend pages listed (per TSV)</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No frontend pages listed (per TSV) — UI coverage unclear</p>
