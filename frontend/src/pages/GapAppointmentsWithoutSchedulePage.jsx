...
// Gap (AI): Appointments without '/schedule
// Appointments without /schedule-optimization (balance clinic capacity, no-show prediction)
import React, { useState } from 'react';

export default function GapAppointmentsWithoutSchedulePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-appointments-without-schedule/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Appointments without '/schedule</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Appointments without /schedule-optimization (balance clinic capacity, no-show prediction)</p>
