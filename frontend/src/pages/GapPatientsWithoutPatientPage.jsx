...
// Gap (AI): Patients without '/patient
// Patients without /patient-risk-score (predict disease progression, glaucoma risk)
import React, { useState } from 'react';

export default function GapPatientsWithoutPatientPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-patients-without-patient/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Patients without '/patient</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Patients without /patient-risk-score (predict disease progression, glaucoma risk)</p>
