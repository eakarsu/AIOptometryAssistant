...
// Gap (Non-AI): No RBAC beyond auth
// No RBAC beyond auth
import React, { useState } from 'react';

export default function GapNoRbacBeyondAuthPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-rbac-beyond-auth/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No RBAC beyond auth</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No RBAC beyond auth</p>
