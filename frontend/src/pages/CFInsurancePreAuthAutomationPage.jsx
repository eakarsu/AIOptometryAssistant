...
// Custom Feature: Insurance pre-auth automation
// Insurance pre-auth automation: Detect insurance-dependent treatments; auto-generate and submit pre-auth requests; track approval status
import React, { useState } from 'react';

export default function CFInsurancePreAuthAutomationPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-insurance-pre-auth-automation/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Insurance pre-auth automation</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Insurance pre-auth automation: Detect insurance-dependent treatments; auto-generate and submit pre-auth requests; track approval status</p>
